...
if args.func == "generate_depth_npy":
	...
	odom_eval.eval_seqs = [9,10] # Seq 03 is missing since the dataset is not available in KITTI homepage.
	pred_dir = args.odom_result_dir
	path = os.listdir(pred_dir)
	for p in path:
			print('eval ' + 'of ' + p)
			odom_eval.eval(args.odom_result_dir+'/'+p)
