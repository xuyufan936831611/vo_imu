...
parser.add_argument('--predicted_inv_depth_path', type=str,   help='path to estimated disparities',      required=True)
...
if __name__ == '__main__':
    ...
    pred_disparities = np.load(args.predicted_inv_depth_path)
    ...
    if args.split == 'kitti':
        ...
        num_samples = 697
        test_files = read_text_lines('./data/depth_evaluation/kitti_eigen/test_files_eigen.txt')
        ...
        print "Getting ground truth depths and predicted depths..."
        for t_id in range(num_samples):
            camera_id = cams[t_id]  # 2 is left, 3 is right
            depth = generate_depth_map(gt_calib[t_id], gt_files[t_id], im_sizes[t_id], camera_id, False, True)
            gt_depths.append(depth.astype(np.float32))

            inv_depth_pred = cv2.resize(pred_disparities[t_id], (im_sizes[t_id][1], im_sizes[t_id][0]), interpolation=cv2.INTER_LINEAR)
            

            # ----------------------------------------------------------------------
            # Convert disparity into depth
            # ----------------------------------------------------------------------
            # inv_depth_pred = inv_depth_pred * inv_depth_pred.shape[1]
            # focal_length, baseline = get_focal_length_baseline(gt_calib[t_id], camera_id)
            # depth_pred = (baseline * focal_length) / (inv_depth_pred+1e-4)

            # ----------------------------------------------------------------------
            # Convert inverse depth to depth
            # ----------------------------------------------------------------------
            depth_pred = 1.0 / (inv_depth_pred+1e-4)
            depth_pred[np.isinf(depth_pred)] = 0

            pred_depths.append(depth_pred)
        print "Getting ground truth depths and predicted depths... Done!"
