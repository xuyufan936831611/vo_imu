...
def show_gt_and_pred(gt,depth_pred):
    mean = np.mean(gt)
    std = np.std(gt)
    std_adj = np.maximum(std, 1.0 / np.sqrt(gt.size))
    img_gt = np.multiply(np.subtract(gt, mean), 1 / std_adj)*255.

    mean = np.mean(depth_pred)
    std = np.std(depth_pred)
    std_adj = np.maximum(std, 1.0 / np.sqrt(depth_pred.size))
    img_pred = np.multiply(np.subtract(gt, mean), 1 / std_adj)*255.
    cv2.imshow('pred', img_gt)
    cv2.waitKey(500)
    cv2.imshow('ground_truth', img_pred)
    cv2.waitKey(500)
