...
flags.DEFINE_string("ckpt_dir", None, "checkpoint file")
FLAGS = flags.FLAGS
...
def main():
    ...
    for test_seq in test_seq:
        seq_dir = os.path.join(FLAGS.dataset_dir, 'sequences', '%.2d' % test_seq)
        img_dir = os.path.join(seq_dir, 'image_2')
        N = len(glob(img_dir + '/*.png'))
        test_frames = ['%.2d %.6d' % (test_seq, n) for n in range(N)]
        with open(FLAGS.dataset_dir + 'sequences/%.2d/times.txt' % test_seq, 'r') as f:
            times = f.readlines()
        ...
        for ckpt_file in checkpoint_files:
            ...
            with tf.Session(config=config) as sess:
                ...
                for tgt_idx in range(N):
                    ...
                    if not os.path.isdir(output_file+'/'+ '%.2d' % test_seq):
                        os.makedirs(output_file+'/'+ '%.2d' % test_seq)
                    out_file = output_file+'/'+ '%.2d' % test_seq+ '/' + '%.6d.txt' % (tgt_idx - max_src_offset)
    ...
    #             out_file = output_file+'/' + '%.6d.txt' % (tgt_idx - max_src_offset)
    #             dump_pose_seq_TUM(out_file, pred_poses, curr_times)
