...
def pose_vec_to_mat(vec):
    ...
    return Tmat
def lidaishu_to_mat(vec):
    fai = np.array(vec[:3]).reshape((3,1))
    theta = np.linalg.norm(fai)
    a = fai / theta
    rou = np.array(vec[3:]).reshape((3,1))
    a_hat = np.array([0, -a[2], a[1],
                      a[2], 0, -a[0],
                      -a[1], a[0], 0]).reshape([3, 3])
    exp_fai_hat = np.cos(theta) * np.eye(3) + (1 - np.cos(theta)) * np.matmul(a, np.transpose(a)) + \
                  np.sin(theta) * a_hat
    J = np.sin(theta) / theta * np.eye(3) + (1 - np.sin(theta) / theta) * np.matmul(a, np.transpose(a)) + \
               (1 - np.cos(theta)) / theta * a_hat
    t = np.matmul(J, rou)
    ones = np.array([0, 0, 0, 1]).reshape([1, 4])
    T = np.vstack((np.hstack((exp_fai_hat, t)), ones))
    return T
def dump_pose_seq_TUM(out_file, poses, times):
    # First frame as the origin
    first_pose = pose_vec_to_mat(poses[0])
    # first_pose = lidaishu_to_mat(poses[0])
    with open(out_file, 'w') as f:
        for p in range(len(times)):
            ...
            #    this_pose = pose_vec_to_mat(poses[p])
            #else:
            #    this_pose = lidaishu_to_mat(poses[p])
