...
def dump_example(n):
    ...
    with open(dump_cam_file, 'w') as f:
        f.write('%f,0.,%f,0.,%f,%f,0.,0.,1.,%f' % (fx, cx, fy, cy, bf))

def main():
    if not os.path.exists(args.dump_root):      #dir to save processed images
        os.makedirs(args.dump_root)

    global data_loader
    if args.dataset_name == 'kitti_odom':
        from kitti.kitti_odom_loader_new import kitti_odom_loader
    ...
    if args.dataset_name == 'kitti_raw_eigen':
        from kitti.kitti_raw_loader_new import kitti_raw_loader
